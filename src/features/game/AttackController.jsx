...
const DiceReadOnly = (props) => {
  ...
  return (
    <div
      className={`dice-container-readonly ${`dice-container-readonly-${player}`}`}
    >
      {dieComponents}
    </div>
  );
...
const AttackButton = (props) => {
  ...
  return (
    <Button
      className="btn btn-inline btn-attack"
      text="Attack"
      disabled={disabled}
      onClick={handleClick}
    />
...
const AttackController = (props) => {
  ...
  return (
    <div className={`attack-controller attack-controller-${currentPlayer}`}>
      ...
      <div
        className={`dices-container ${`dices-container-${togglePlayer(
          currentPlayer
        )}`}`}
      >
        {attackPossible && !isLastAttack ? (
